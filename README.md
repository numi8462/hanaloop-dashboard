...
탄소 배출량을 측정하고 관리하는 B2B SaaS 플랫폼입니다.
...
| 기능           | 설명                                              |
| -------------- | ------------------------------------------------- |
| PCF 대시보드   | KPI 카드, 파이 차트, 월별 트렌드 차트             |
| 데이터 관리    | 활동 데이터 추가/수정/삭제, 실시간 유효성 검증    |
| 배출계수 관리  | GHG Scope 분류, 버전 이력 추적                    |
| 탄소 비용 계산 | K-ETS 배출권 기준 예상 비용, 카테고리별 상세 내역 |
| 목표 관리      | 연도별 감축 목표 설정, 진행률 시각화              |
| 자동 보고서    | Excel 형식으로 PCF 데이터 자동 추출               |
| AI 어시스턴트  | Groq API 기반 탄소 데이터 분석 챗봇               |
...
| POST   | `/api/goals`                        | 목표 생성 또는 수정 (upsert)        |
| DELETE | `/api/goals/:id`                    | 목표 삭제                           |
| POST   | `/api/chat`                         | AI 어시스턴트 답변 생성             |
...
### AI 어시스턴트 — Groq API 선택

Claude API, Gemini API 등을 검토했지만 무료로 사용 가능한 Groq API (llama-3.1-8b-instant)를 선택했습니다. 현재 PCF 데이터를 시스템 프롬프트에 컨텍스트로 전달해서 데이터 기반 질문에 답변합니다. 실제 서비스라면 더 강력한 모델로 교체할 수 있습니다.
...
- **AI 모델 업그레이드**: 현재 Groq의 llama-3.1-8b-instant를 사용하고 있는데, Claude나 GPT-4 같은 더 강력한 모델로 교체하면 답변 품질이 크게 향상됩니다.
...
이번 과제를 진행하면서 Claude를 적극적으로 활용했습니다. 단순히 작업 속도를 높이기 위한 목적이 아니라, 제한된 시간 내에 더 높은 완성도의 결과물을 만들기 위한 도구로 활용했습니다.

특히 반복적인 코드 작성이나 보일러플레이트 생성 과정에서 시간을 절약하고, 그 시간을 전체적인 구조 설계와 사용자 경험 개선에 집중하는 데 사용했습니다.

AI 도구를 사용할 때는 생성된 코드를 그대로 사용하는 것이 아니라, 항상 직접 검토하고 실행해보면서 의도한 동작과 일치하는지 확인했습니다.
...
- _"Groq API를 사용해서 현재 PCF 데이터를 컨텍스트로 전달하고 탄소 배출량 관련 질문에 답변하는 AI 챗봇을 만들어줘"_
...
**색상 테마 및 UI 디자인**
AI의 기본적인 UI 느낌을 벗어나기 위해 컴포넌트의 UI나 색깔 테마를 직접 수정하면서 최대한 간편하면서도 편리하게 쓸 수 있는 느낌으로 구현했습니다.

**차트**
누적 바 그래프와 그룹 그래프 중 둘 다 사용자에 따라 보고 싶은 방식을 고를 수 있게 하는 것이 더 좋은 판단이라 생각하여 누적/그룹 토글 기능을 추가했습니다.
...
| 작업                                              | 시간     | 비고                                                       |
| ------------------------------------------------- | -------- | ---------------------------------------------------------- |
| 프로젝트 설계 및 환경 설정                        | 약 1시간 | Next.js, Prisma, Tailwind 세팅                             |
| DB 스키마 및 API 구현                             | 약 2시간 | Prisma 스키마, API Routes, DB 쿼리 레이어                  |
| PCF 계산 로직 및 상태 관리                        | 약 2시간 | pcf-calculator, Zustand store, hooks, services 레이어 분리 |
| 대시보드 UI 구현                                  | 약 3시간 | KPI 카드, 파이 차트, 월별 차트                             |
| 데이터 관리 페이지 구현                           | 약 2시간 | ActivityForm, ActivityTable, 수정/삭제 기능                |
| 배출계수 관리 페이지 구현                         | 약 1시간 | 배출계수 목록 및 버전 이력                                 |
| 반응형 작업                                       | 약 2시간 | 모바일 사이드바, 테이블 스크롤, 카드 레이아웃              |
| Docker 설정 및 디버깅                             | 약 2시간 | Prisma 버전 이슈, 시드 데이터 컴파일 문제 해결             |
| 기능 확장 (탄소 비용, 목표 관리, 보고서, AI 챗봇) | 약 4시간 | Supabase 연결, Vercel 배포 포함                            |
| README 작성                                       | 약 1시간 | 설계 설명, trade-off, AI 사용 내역                         |

### 시간이 많이 소요된 부분

**Docker 설정**
Prisma 버전 충돌 문제가 가장 까다로웠습니다. 로컬에서는 Prisma 5가 설치되어 있는데 Docker 빌드 시 Prisma 7이 설치되면서 스키마 문법 오류가 발생했습니다. runner 스테이지에 Prisma 바이너리를 직접 복사하고, seed.ts를 JS로 컴파일하는 방식으로 해결했습니다.

**서비스 레이어 설계**
처음에는 API Route에서 Prisma를 직접 호출하는 방식으로 시작했다가 services / store / hooks / lib/db 4개 레이어로 분리하는 과정에서 데이터 흐름을 다시 정리하는 데 시간이 걸렸습니다.

**UI/UX 설계**
사용자 모두에게 사용하기 편하면서 접근하기 쉬운 UX를 고려해야 했기 때문에 계속해서 직접 테스팅하면서 수정해야 했습니다. 하나루프의 대시보드 디자인도 참고하면서 진행했습니다.
